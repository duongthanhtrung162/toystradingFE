// import { take, call, put, select } from 'redux-saga/effects';
import { take, call, put, select, takeLatest } from 'redux-saga/effects';
 import * as _ from 'lodash';
 import * as constant from './constants'
 import * as PageActions from './actions';
import apiRequests from '../App/apiRequest'
// Individual exports for testing
export function* getUserList(action) {
  const { reject, resolve } = action.payload;
  
  try {
    const apiUrl = yield apiRequests.userAuth;
    const result = yield apiRequests.getRequest(apiUrl, {});
    
    if (result) {
      yield put(PageActions.defaultAction('userList', result.data.data));
      resolve(result)
     
    }
  } catch (err) {
    reject(err);
  }
}
export function* deleteUser(action) {
  const { reject, resolve, data } = action.payload;
  
  try {
    const apiUrl = yield apiRequests.userAuth;
    const result = yield apiRequests.deleteRequest(`${apiUrl}/${data}`, {});
    
    if (result) {
      resolve(result)
     
    }
  } catch (err) {
    reject(err);
  }
}
export default function* userViewSaga() {
  yield takeLatest(constant.ACTION_GET_USER, getUserList);
  yield takeLatest(constant.ACTION_DELETE_USER, deleteUser);

}
